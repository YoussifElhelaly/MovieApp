...
export default function MovieDetails() {
    ...
    useEffect(() => {
        fetch(`https://api.themoviedb.org/3/tv/${movieId?.index}?api_key=122131e3599c6bef391494f36765eaa0&language=en-US`)
        .then((res) => res.json())
        .then((data) => SetMovie(data))

        fetch(`https://api.themoviedb.org/3/tv/${movieId?.index}/credits?api_key=122131e3599c6bef391494f36765eaa0&language=en-US`)
        .then((res) => res.json())
        .then((data) => SetCredits(data.cast))

        fetch(`https://api.themoviedb.org/3/tv/${movieId?.index}/reviews?api_key=122131e3599c6bef391494f36765eaa0&language=en-US&page=1`)
        .then((res) => res.json())
            ...
            <div className={`${styles.img} img w-full h-[400px] md:h-[500px] relative`}>
                ...
                <div className="container mx-auto xl:px-20 lg:py-40 py-5 md:justify-around relative z-[40] flex lg:justify-evenly md:flex-row flex-col">
                    <div className="poster w-2/4 md:w-1/4 sticky top-[100px] h-full">
                        ...
                        <div className="add">
                            <button className="block bg-[#dd003f] font-bold text-md  w-full my-2 py-3 rounded">Watch later</button>
                            <button className="block bg-[#dcf836] font-bold text-md text-black w-full my-2 py-3 rounded">ADD TO FAVORITE</button>
                        </div>
                    </div>
                    <div className="details w-[90%] md:w-[50%]">
                        <h3 className="text-4xl font-bold">{movie.original_name}<span className="year text-2xl ml-5 font-normal text-gray-400">{movie.first_air_date?.split("-")[0]}</span></h3>
                        ...
                        <div className="cast my-10">
                            ...
                            {
                                credits?.map((actor)=>{
                                    let srcIMG = `https://image.tmdb.org/t/p/original${actor.profile_path} `
                                    if(typeof(actor.profile_path) === "object"){
                                        srcIMG = "https://www.themoviedb.org/assets/2/v4/glyphicons/basic/glyphicons-basic-4-user-grey-d8fe957375e70239d6abdd549fd7568c89281b2179b5f4470e2e12895792dfa5.svg"
                                    }
                                    return (
                                        <div className="actor flex justify-between mb-5 items-center">
                                            <div className="info flex items-center">
                                                <img src={srcIMG} className="w-[60px] mr-5 h-[60px] object-cover rounded"/>
